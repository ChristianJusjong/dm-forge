...
- Include "count" field (default 1)`;
        ...
        if (USE_GROQ && apiKey) {
            try {
                ...
                console.log('✅ Generated with Groq (fast & unlimited)');
            } catch (groqError) {
                console.warn('⚠️ Groq failed, using Gemini:', groqError.message);
                text = await generateWithGemini(prompt);
                console.log('✅ Generated with Gemini (fallback)');
            }
        } else {
            // Use Gemini by default
            console.log('ℹ️ Using Gemini (no Groq API key)');
            text = await generateWithGemini(prompt);
      ...
      <div class="card" style="padding: 1rem; background: linear-gradient(135deg, var(--parchment-dark) 0%, var(--parchment) 100%); border-left: 4px solid var(--gold);">
        ${backstory ? `
          <div style="margin-bottom: 0.75rem;">
            <strong style="color: var(--wood-brown); display: block; margin-bottom: 0.25rem;">📖 Backstory:</strong>
            <p style="color: var(--color-text-primary); margin: 0; line-height: 1.5;">${backstory}</p>
          </div>
        ` : ''}
        ${purpose ? `
          <div>
            <strong style="color: var(--wood-brown); display: block; margin-bottom: 0.25rem;">🎯 Purpose:</strong>
      ...
      <div class="encounter-card">
        <div class="encounter-header">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="btn-small btn-danger delete-encounter-btn" data-id="${encounter.id}">×</button>
          </div>
        </div>
        ...
        ${encounter.creatures.length > 0 ? `
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1.5rem; padding-top: 1rem; border-top: 2px solid var(--gold);">
            ...
            <div style="display: flex; gap: 0.5rem;">
              <button class="btn btn-secondary save-encounter-notes-btn" data-id="${encounter.id}">📝 Save to Notes</button>
...
function loadEncounterToInitiative(encounterId) {
    ...
    const totalCreatures = encounter.creatures.reduce((sum, c) => sum + (c.count || 1), 0);
    if (confirm(`${totalCreatures} creatures indlæst! Gå til Initiativ Tracker?`)) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadEncounters();
    renderEncounters(); // Initial Render
    ...
    document.querySelectorAll('.modal-overlay').forEach(overlay => {
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay) {
                ...
                // Reset specific modals if needed
                if (overlay.id === 'new-encounter-modal') hideNewEncounterModal();
                if (overlay.id === 'edit-encounter-modal') currentEditingEncounter = null;
            }
        });
    });
});
